...
class GoogleSearch:
    def __init__(self, overwrite: int, input_file: str, output_dir: str, max_page: int, exceptions_file: str):
        ...
        self.logger = self.utils.set_colorful_logging("Google Search")  # Set up logging
        self.websites = pd.read_csv(self.input_file, low_memory=False)  # Load websites as DataFrame
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Search Google for websites and save results.')
    ...
    parser.add_argument('--exceptions', type=str, default='exceptions/google.csv', help='CSV file to log exceptions')
    
    args = parser.parse_args()
    
    google_search = GoogleSearch(args.overwrite, args.input, args.output, args.max_page, args.exceptions)
    google_search.run()
